...
public static class SplashView
{
    ...
    public static void Show(ColorScheme scheme, Window mainWindow)
    {
        ...
        var magentaScheme = new ColorScheme() { Normal = Application.Driver.MakeAttribute(Color.BrightMagenta, Color.Black) };
        ...
        Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(fillInterval), _ =>
        {
            filled++;
            labels[barRow].Text =
                "[" +
                new string('█', filled) +
                new string(' ', barSlots - filled) +
                "]";
            ...
            Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(0.5), __ =>
            {
                ...
                Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(0.3), ___ =>
                {
                    for (int j = 0; j <= 2; j++) labels[j].Visible = false;
                    splash.SetNeedsDisplay();
                    return false;
                });

                // Second pulse at 0.8s
                Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(0.8), ___ =>
                {
                    for (int j = 0; j <= 2; j++)
                    {
                        labels[j].ColorScheme = magentaScheme;
                    ...
                    // Hide arcs after 0.3s
                    Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(0.3), ____ =>
                    {
                        for (int j = 0; j <= 2; j++) labels[j].Visible = false;
                        splash.SetNeedsDisplay();
                        return false;
                ...
                Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(1.2), ___ =>
                {
                    ...
                    // Jitter loop
                    Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(jitterInterval), ____ =>
                    {
                        jitterCount++;
                        ...
                        Application.MainLoop.AddTimeout(TimeSpan.FromSeconds(2.0), _____ =>
                        {
                            top.Remove(splash);
