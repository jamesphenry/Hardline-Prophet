...
public class InGameView : View // Using View instead of Window to embed in MainWindow
{
    ...
    public InGameView()
    {
        ...
        var statDivider = new LineView(Orientation.Horizontal) { X = 1, Y = Pos.Bottom(_creditsLabel) + 1, Width = Dim.Fill(1) };
        ...
        var traceDivider = new LineView(Orientation.Horizontal) { X = 1, Y = Pos.Bottom(_dataYieldLabel) + 1, Width = Dim.Fill(1) };
        // Initialize the Trace Level label
        _traceLevelLabel = new Label("Trace Level: ?") { X = 1, Y = Pos.Bottom(traceDivider) + 1 };

        statusFrame.Add(
            _usernameLabel, _levelLabel, _xpLabel, _creditsLabel,
            statDivider,
            _hackSpeedLabel, _stealthLabel, _dataYieldLabel,
            traceDivider,
            _traceLevelLabel
         );
        ...
        { X = Pos.Right(statusFrame) + 1, Y = 0, Width = Dim.Fill(1), Height = 5 };
        _missionLabel = new Label("Mission: None") { X = 1, Y = 1 };
        _progressBar = new ProgressBar() { X = 1, Y = Pos.Bottom(_missionLabel), Width = Dim.Fill(1), Height = 1, Fraction = 0f, ColorScheme = Colors.Base };
        missionFrame.Add(_missionLabel, _progressBar);

        // --- Log Window ---
        var logFrame = new FrameView("Log")
        { X = Pos.Right(statusFrame) + 1, Y = Pos.Bottom(missionFrame), Width = Dim.Fill(1), Height = Dim.Fill(1) };
        _logView = new TextView() { X = 0, Y = 0, Width = Dim.Fill(), Height = Dim.Fill(), ReadOnly = true, WordWrap = true };
    ...
    public void UpdateState(GameState? state)
    {
        ...
        { missionName = missionDef.Name; if (missionDef.DurationTicks > 0) { progressFraction = (float)state.ActiveMissionProgress / missionDef.DurationTicks; } else { progressFraction = state.ActiveMissionProgress > 0 ? 1f : 0f; } }
    ...
    public void AddLogMessage(string message)
    {
        ...
        var timestamp = DateTime.Now.ToString("HH:mm:ss"); _logMessages.Add($"[{timestamp}] {message}"); while (_logMessages.Count > MaxLogLines) { _logMessages.RemoveAt(0); }
        _logView.Text = string.Join(Environment.NewLine, _logMessages); Application.MainLoop.Invoke(() => { if (_logMessages.Any()) { int desiredTopRow = Math.Max(0, _logMessages.Count - _logView.Bounds.Height); _logView.TopRow = desiredTopRow; _logView.SetNeedsDisplay(); } });
    }
}
