...
public class JsonGameStateRepository : IGameStateRepository
{
    ...
    {
        _isDevMode = isDevMode; if (string.IsNullOrWhiteSpace(basePath)) { string appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData); _saveBasePath = Path.Combine(appDataPath, "HardlineProphet", "Saves"); } else { _saveBasePath = basePath; }
        Directory.CreateDirectory(_saveBasePath); Console.WriteLine($"Repository Initialized. Dev Mode: {_isDevMode}. Path: {_saveBasePath}");
    }
    ...
    public async Task<GameState> LoadStateAsync(string username)
    {
        ...
        if (!File.Exists(filePath)) { return new GameState { Username = username }; }
        ...
        try
        {
            using (JsonDocument document = JsonDocument.Parse(json)) { if (document.RootElement.TryGetProperty("Version", out JsonElement versionElement) && versionElement.TryGetInt32(out int version)) { fileVersion = version; } }

            GameState finalState;

            if (fileVersion < GameConstants.CurrentSaveVersion)
            {
                Console.WriteLine($"Detected old save version V{fileVersion} for user '{username}'. Migrating...");
                ...
                Console.WriteLine($"Migration complete for user '{username}' to V{finalState.Version}. Final checksum: {finalState.Checksum}");
            ...
            {
                finalState = JsonSerializer.Deserialize<GameState>(json, _deserializeOptions)
                    ?? throw new InvalidDataException($"Failed to deserialize V{fileVersion} save file content (result was null).");
    ...
    private GameState MigrateV1ToV2(string username, string v1Json)
    {
        ...
        var stateV2 = migratedState with { Version = 2 }; // Set version to 2
    ...
    private GameState MigrateV2ToV3(string username, string v2Json)
    {
        ...
        if (migratedState == null) throw new InvalidDataException($"Failed to deserialize V2 save file during migration for user '{username}'.");

        // Create the final V3 state *before* calculating checksum
        var stateV3 = migratedState with { Version = GameConstants.CurrentSaveVersion }; // Set version to 3
    ...
    private GameState MigrateV2ToV3(GameState stateV2) // Overload for chaining from V1->V2
    {
        ...
        var stateV3 = stateV2 with { Version = GameConstants.CurrentSaveVersion }; // Set version to 3
        string checksumV3 = ComputeChecksum(stateV3); // Calculate checksum based on V3 state
        return stateV3 with { Checksum = checksumV3 }; // Return V3 state with V3 checksum
    }
    ...
    private void ValidateChecksum(GameState stateToValidate, string username)
    {
        ...
        if (_isDevMode) { Console.WriteLine($"Dev Mode: Skipping checksum validation for user '{username}'."); if (!string.IsNullOrEmpty(stateToValidate.Checksum)) { string expectedChecksum = ComputeChecksum(stateToValidate); if (stateToValidate.Checksum != expectedChecksum) { Console.WriteLine($"Dev Mode WARNING: Checksum mismatch for user '{username}'. File checksum: '{stateToValidate.Checksum}', Calculated: '{expectedChecksum}'. Loading anyway."); } else { Console.WriteLine($"Dev Mode: Checksum is valid for user '{username}'."); } } else { Console.WriteLine($"Dev Mode: No checksum present for user '{username}'. Loading anyway."); } return; }
        if (string.IsNullOrEmpty(stateToValidate.Checksum)) { throw new InvalidDataException($"Save file integrity check failed for user '{username}' (missing checksum in V{stateToValidate.Version} file)."); }
        string expectedChecksumProd = ComputeChecksum(stateToValidate); if (stateToValidate.Checksum != expectedChecksumProd) { throw new InvalidDataException($"Save file integrity check failed for user '{username}' (checksum mismatch)."); }
    ...
    public async Task SaveStateAsync(GameState gameState)
    {
        ...
        if (gameState == null) throw new ArgumentNullException(nameof(gameState)); if (string.IsNullOrWhiteSpace(gameState.Username)) throw new ArgumentException("GameState must have a valid Username to save.", nameof(gameState)); var filePath = GetSaveFilePath(gameState.Username); string calculatedChecksum = ComputeChecksum(gameState); var stateToSerialize = gameState with { Version = GameConstants.CurrentSaveVersion, Checksum = calculatedChecksum }; /* Ensure saving current version */ try { string json = JsonSerializer.Serialize(stateToSerialize, _serializeOptionsSave); await File.WriteAllTextAsync(filePath, json); } catch (JsonException jsonEx) { throw new Exception($"Failed to serialize game state for user '{gameState.Username}'.", jsonEx); } catch (IOException ioEx) { throw new IOException($"Failed to write save file for user '{gameState.Username}'.", ioEx); }
    }
