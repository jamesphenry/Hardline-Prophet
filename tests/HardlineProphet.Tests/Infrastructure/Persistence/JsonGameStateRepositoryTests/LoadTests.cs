...
public class LoadTests : PersistenceTestsBase
{
    ...
    public async Task LoadStateAsync_WhenSaveFileExistsAndIsValid_ShouldReturnDeserializedGameState()
    {
        ...
        var originalState = new GameState
        {
            ...
            Experience = 1234.5,
            Credits = 5000,
            Stats = new PlayerStats { HackSpeed = 10, Stealth = 8, DataYield = 2 }, // Create specific stats
    ...
    {
        // ... (test remains the same) ...
        var username = "DevInvalidChecksum"; var testDir = GetTestDirectoryPath(); var repository = new JsonGameStateRepository(basePath: testDir, isDevMode: true); var originalState = new GameState { Username = username, Level = 9, Credits = 900, Version = GameConstants.CurrentSaveVersion }; var stateToSave = originalState with { Checksum = "THIS_IS_WRONG======" }; var saveFilePath = GetSaveFilePath(username); var jsonToSave = JsonSerializer.Serialize(stateToSave, _saveTestSerializerOptions); await File.WriteAllTextAsync(saveFilePath, jsonToSave); _output.WriteLine($"Saved file with INCORRECT checksum for dev mode test at: {saveFilePath}"); GameState loadedState = null!; Exception caughtException = null!; try { loadedState = await repository.LoadStateAsync(username); } catch (Exception ex) { caughtException = ex; _output.WriteLine($"Caught UNEXPECTED exception in dev mode: {ex.GetType().Name} - {ex.Message}"); }
        Check.That(caughtException).IsNull(); Check.That(loadedState).IsNotNull(); Check.That(loadedState.Username).IsEqualTo(originalState.Username); Check.That(loadedState.Level).IsEqualTo(originalState.Level); Check.That(loadedState.Checksum).IsEqualTo("THIS_IS_WRONG======");
    }
    ...
    {
        // ... (test remains the same) ...
        var username = "DevMissingChecksum"; var testDir = GetTestDirectoryPath(); var repository = new JsonGameStateRepository(basePath: testDir, isDevMode: true); var originalState = new GameState { Username = username, Level = 11, Credits = 1100, Version = GameConstants.CurrentSaveVersion, Checksum = null }; var saveFilePath = GetSaveFilePath(username); var jsonToSave = JsonSerializer.Serialize(originalState, _saveTestSerializerOptions); await File.WriteAllTextAsync(saveFilePath, jsonToSave); _output.WriteLine($"Saved file with MISSING checksum for dev mode test at: {saveFilePath}"); GameState loadedState = null!; Exception caughtException = null!; try { loadedState = await repository.LoadStateAsync(username); } catch (Exception ex) { caughtException = ex; _output.WriteLine($"Caught UNEXPECTED exception in dev mode: {ex.GetType().Name} - {ex.Message}"); }
        Check.That(caughtException).IsNull(); Check.That(loadedState).IsNotNull(); Check.That(loadedState.Username).IsEqualTo(originalState.Username); Check.That(loadedState.Level).IsEqualTo(originalState.Level); Check.That(loadedState.Checksum).IsNull();
    }
}
